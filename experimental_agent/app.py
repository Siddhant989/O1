...
def display_pdf_images():
    ...
    if not st.session_state.pdf_images:
        st.info("No images have been extracted from PDFs yet. Please upload a PDF file first.")
        return False
        ...
        if images:
            ...
            for img_data in images:
                try:
                    ...
                    st.warning(f"Could not display image from page {img_data['page']}: {str(e)}")
...
def process_audio(audio_file, operations=None):
    ...
    try:
        ...
        if any(op in operations for op in ['noise_reduction', 'normalize', 'trim_silence']):
...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_files = st.file_uploader(
            ...
            type=['pdf', 'txt', 'xlsx', 'xls', 'csv', 'jpg', 'jpeg', 'png', 
                  'py', 'js', 'jsx', 'ts', 'tsx', 'java', 'cpp', 'c', 'cs', 
                  'php', 'rb', 'go', 'rs', 'swift', 'kt', 'scala', 'html', 
                  'css', 'sql', 'sh', 'md', 'json', 'xml', 'yaml', 'yml', 
                  'toml', 'ini', 'env', 'ipynb', 'wav', 'mp3', 'ogg', 'flac'],
        ...
        if uploaded_files:
            for file in uploaded_files:
                if file.type.startswith('image/'):
                    ...
                    with col2:
                        if st.button(
                            "👁️ Show" if not st.session_state.preview_states[file.name] else "👁️ Hide",
                            key=button_key,
                            use_container_width=True
                        ):
                            # Toggle the state
                            st.session_state.preview_states[file.name] = not st.session_state.preview_states[file.name]
                            st.rerun()
        ...
        if process_button:
            with st.spinner("Processing..."):
                    ...
                    for file in uploaded_files:
                        try:
                            ...
                            if not file.type.startswith('audio/'):
                                file_text = st.session_state.agent.get_text_from_file(file)
                                if file_text:
                                    all_text += file_text + "\n\n"
                                else:
                ...
                if website_url:
                    try:
                        website_text = asyncio.run(st.session_state.agent.extract_text_from_website_async(website_url))
                        if website_text:
                ...
                if not all_text.strip() and not any(f.type.startswith('audio/') for f in uploaded_files):
                ...
                try:
                    if all_text.strip():
                        ...
                        vector_store = st.session_state.agent.get_vector_store(text_chunks)
                        if not vector_store:
                            st.error("🔴 Failed to create vector store from the content.")
                            st.session_state.content_processed = False
                            return
    ...
    # Chat input
    if st.session_state.content_processed or any(f.type.startswith('audio/') for f in st.session_state.uploaded_files):
        ...
        if user_question:
            ...
            try:
                ...
                if any(f.type.startswith('audio/') for f in st.session_state.uploaded_files):
                    audio_file = next(f for f in st.session_state.uploaded_files if f.type.startswith('audio/'))
                    ...
                    try:
                        ...
                        with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_wav:
                            ...
                            if 'background' in user_question.lower() and 'noise' in user_question.lower():
                                operations.append('background_noise')
                            elif 'noise' in user_question.lower() and 'reduce' in user_question.lower():
                                ...
                                with st.spinner(f"Processing audio..."):
                                    ...
                                    for operation in operations:
                                        if operation == 'background_noise':
                                            ...
                                            processed_audio_data = (processed_y * 32767).astype(np.int16)
                                            # Create AudioSegment
                                            processed_audio = AudioSegment(
                                            ...
                                            response_parts = ["Background noise has been removed"]
                                        
                                        elif operation == 'transcribe':
                                            # If we have processed audio, transcribe that, otherwise transcribe original
                                            audio_to_transcribe = processed_audio if processed_audio else AudioSegment.from_wav(temp_wav.name)
                                            transcription = process_and_transcribe(audio_to_transcribe)
                                            response_parts = ["Here's the transcription:", transcription]
                                        
                                        elif operation == 'noise_reduction':
                                            audio_results = st.session_state.agent.process_audio_input(temp_wav.name, operation)
                                            if 'processed_audio' in audio_results:
                                                processed_audio = audio_results['processed_audio']
                                                response_parts = ["Noise has been reduced"]
                                        
                                        elif operation == 'normalize':
                                            audio_results = st.session_state.agent.process_audio_input(temp_wav.name, operation)
                                            if 'processed_audio' in audio_results:
                                                processed_audio = audio_results['processed_audio']
                                                response_parts = ["Audio has been normalized"]
                                    ...
                                    if processed_audio and 'transcribe' not in operations:
                                        message["audio"] = processed_audio.export().read()
                    ...
                    response = asyncio.run(st.session_state.agent.process_user_input_async(user_question, recent_file_info))
