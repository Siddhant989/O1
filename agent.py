...
class Agent:
    ...
    def process_image(self, image_file):
        try:
            ...
            if image_file.type == "application/pdf":
                ...
                try:
                    ...
                    try:
                        ...
                        raise Exception(f"Both vision model and OCR failed. Vision error: {str(e)}, OCR error: {str(ocr_error)}")
            ...
            if "API key" in str(e):
                ...
                error_msg = "Error: The uploaded file is not a valid image or is corrupted"
    ...
    def check_file_size(self, file):
        ...
        if file_size > self.max_file_size:
            raise Exception(f"File size ({file_size / (1024 * 1024):.2f}MB) exceeds the maximum allowed size of 4MB")
        ...
        try:
            ...
            if file.type == "application/pdf":
                ...
                text = file.getvalue().decode("utf-8")
                             ...
                             "text/csv"]:
                df = pd.read_excel(file) if file.type != "text/csv" else pd.read_csv(file)
                text = df.to_string()
    ...
    def update_memory(self, user_input, assistant_response, context=None):
        ...
        if len(self.memory['document_context']) > 10:
            recent_docs = dict(sorted(self.memory['document_context'].items(), 
                                    key=lambda x: x[0], 
                                    reverse=True)[:10])
    ...
    def get_relevant_context(self, user_question):
        ...
        for conv in reversed(self.memory['conversations']):
            if any(keyword in user_question.lower() for keyword in conv['user_input'].lower().split()):
    ...
    def get_conversational_chain(self):
        ...
        try:
            ...
            prompt = PromptTemplate(template=prompt_template, input_variables=["memory_context", "context", "web_search", "question"])
    ...
    async def process_user_input_async(self, user_question, recent_file_info=None):
        try:
            ...
            if query_type == "visualization":
                return await self._handle_visualization_query(user_question, recent_file_info)
            elif query_type == "comparison":
                return await self._handle_comparison_query(user_question, recent_file_info)
            elif query_type == "prediction":
                return await self._handle_prediction_query(user_question, recent_file_info)
            elif query_type == "analysis":
                return await self._handle_analysis_query(user_question, recent_file_info)
    ...
    def _analyze_query_type(self, question):
        ...
        if any(word in question for word in ['graph', 'plot', 'chart', 'visualize', 'visualization', 'show', 'display']):
            return "visualization"
        
        # Comparison queries
        if any(word in question for word in ['compare', 'difference', 'similar', 'versus', 'vs', 'versus', 'contrast']):
            return "comparison"
        
        # Prediction queries
        if any(word in question for word in ['predict', 'forecast', 'future', 'trend', 'will', 'going to']):
            return "prediction"
        
        # Analysis queries
        if any(word in question for word in ['analyze', 'analysis', 'pattern', 'trend', 'correlation', 'relationship']):
            return "analysis"
    ...
    def _extract_visualization_params(self, question, available_columns):
        ...
        mentioned_columns = [col for col in available_columns if col.lower() in question]
        
        # If no columns mentioned, use default columns
        if not mentioned_columns:
            mentioned_columns = available_columns[:2] if len(available_columns) >= 2 else available_columns
    ...
    def process_user_input(self, user_question, recent_file_info=None):
        return asyncio.run(self.process_user_input_async(user_question, recent_file_info))
        ...
        try:
            ...
            for line in code.split('\n'):
                if line.strip().startswith('import ') or line.strip().startswith('require('):
                    ...
                    analysis['classes'].append(line.strip())
                elif '=' in line and not line.strip().startswith(('//', '/*', 'function', 'class')):
                    analysis['variables'].append(line.strip())
        ...
        try:
            ...
            for line in code.split('\n'):
                if line.strip().startswith('import '):
                    ...
                    analysis['classes'].append(line.strip())
                elif '(' in line and ')' in line and '{' in line and not line.strip().startswith(('//', '/*', 'import', 'class')):
                    analysis['methods'].append(line.strip())
                elif '=' in line and not line.strip().startswith(('//', '/*', 'import', 'class')):
                    analysis['variables'].append(line.strip())
                elif line.strip().startswith('//') or line.strip().startswith('/*'):
                    analysis['comments'].append(line.strip())
    ...
    def _analyze_cpp_code(self, code): return {'error': 'Not implemented'}
    def _analyze_csharp_code(self, code): return {'error': 'Not implemented'}
    def _analyze_php_code(self, code): return {'error': 'Not implemented'}
    def _analyze_ruby_code(self, code): return {'error': 'Not implemented'}
    def _analyze_go_code(self, code): return {'error': 'Not implemented'}
    def _analyze_rust_code(self, code): return {'error': 'Not implemented'}
    def _analyze_swift_code(self, code): return {'error': 'Not implemented'}
    def _analyze_kotlin_code(self, code): return {'error': 'Not implemented'}
    def _analyze_scala_code(self, code): return {'error': 'Not implemented'}
    def _analyze_html_code(self, code): return {'error': 'Not implemented'}
    def _analyze_css_code(self, code): return {'error': 'Not implemented'}
    def _analyze_sql_code(self, code): return {'error': 'Not implemented'}
    def _analyze_shell_code(self, code): return {'error': 'Not implemented'}
    def _analyze_markdown_code(self, code): return {'error': 'Not implemented'}
    def _analyze_json_code(self, code): return {'error': 'Not implemented'}
    def _analyze_xml_code(self, code): return {'error': 'Not implemented'}
    def _analyze_yaml_code(self, code): return {'error': 'Not implemented'}
    def _analyze_toml_code(self, code): return {'error': 'Not implemented'}
    def _analyze_ini_code(self, code): return {'error': 'Not implemented'}
    def _analyze_env_code(self, code): return {'error': 'Not implemented'}
            ...
            if operation == 'transcribe':
                ...
                with sr.AudioFile(audio_file) as source:
                    ...
                    try:
                        ...
                        results['error'] = f"Error with speech recognition service: {str(e)}"
                ...
                samples = (reduced_noise * np.iinfo(samples.dtype).max).astype(samples.dtype)
    ...
    def should_respond_with_audio(self, user_input):
        ...
        audio_keywords = [
            'speak', 'say', 'tell me', 'pronounce', 'read', 'narrate',
            'voice', 'audio', 'sound', 'listen', 'hear'
        ]
